...
この文書は、配置済みの設計資料と現在のソースコードを照合した実装計画です。v2 スキーマの採用や JSON の移行を、この文書の追加だけで実施したものとは扱いません。今回の作業範囲は現状把握、ブランチ作成、計画策定です。
...
| [バージョンレジストリ案](../../src/data/engine-versions-v2-draft.json) | 現在は本番処理から参照されていないデータ案 |
