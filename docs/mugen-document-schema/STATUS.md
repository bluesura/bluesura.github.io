...
- `npm run mugen:test`: 25件。モデル、元フィールド保存、対象外原本の不変性、新しい出典のローカルファイル・アンカー、dry-run・一括事前検証・再実行保護を検証。公開文の優先順位と、訂正前の移行内容・CNS 出力の保存も検証
...
最初の小分け移行 `axis-motion-01` は [BATCH_MIGRATION.md](BATCH_MIGRATION.md) に記録しています。VelAdd / VelSet の「乗算」「ターゲット」という旧説明は、原文を保持する [説明の訂正手順](EDITORIAL_GUIDE.md) を整えて修正しました。PosSet の座標基準、PosAdd の数値範囲は引き続き保留しています。次は VelMul / PosFreeze / Gravity を候補に、旧 JSON と資料の照合から次の小分け計画を作成します。全件の履歴を機械的に変更履歴へ置き換えることはしません。
