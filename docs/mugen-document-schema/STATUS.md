...
段階0〜4の基盤実装、主要11件・補助5件に加え、小分け移行14件への v2 任意構造の追加が完了しました。累計30件です。旧フィールドは削除していません。全体の移行および MUGEN の実機検証が完了したという意味ではありません。

| 段階 | 状態 |
| --- | --- |
| 原本保存 | `2e12ca38` に配置済み資料を保存 |
| 比較基準 | 初回24ページ・共通2項目・261 URL・全258原本のハッシュを固定。追加14ページは別の比較基準へ保存 |
...
| 全体展開 | 4バッチ14件を実施。対象指定・dry-run・再実行保護を追加。対象内の残り204件 |
...
- Trigger 11件: MoveContact、AnimElem、IfElse、Cond、AILevel、StandBy、Const、Acos、Asin、Atan、PI
...
- `npm run mugen:test`: 31件。モデル、元フィールド保存、対象外原本の不変性、新しい出典のローカルファイル・アンカー、dry-run・一括事前検証・再実行保護を検証。公開文の優先順位、概要・内部の資料差、コード例の公開制御、CNS 出力も検証
- `npm run build`: 263ページを生成
- `npm run mugen:check-html`: 261 URL、初回24ページ・追加14ページ・一覧 等3ページ。公開本文・メタ情報・履歴・セクション・出典・画像参照の保存、内部記録・内部コード例の非表示、共通項目と CNS コピペ出力を検証
...
`npm run mugen:inventory` で全件を再集計できます。現在、対象内の旧形式204件に未対応履歴300項目があります。また、28ページに現在の表示コンポーネントが使っていない旧フィールドがあります。未表示情報も原本に保持しています。内部注記は20件です。

小分け移行の記録は [BATCH_MIGRATION.md](BATCH_MIGRATION.md) にあります。Acos / Asin / Atan / PI まで実施し、原文を残す説明訂正をページ概要へ、公開制御を既存コード例へ拡張しました。Gravity / PosFreeze の相互作用、PosSet の座標基準、PosAdd の数値範囲、Atan / PI の非公開例の訂正は引き続き内部で追跡します。次の関連グループは既存資料と旧 JSON を確認してから選定します。
